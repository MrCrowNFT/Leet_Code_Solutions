...
class Solution:
    def calPoints(self, operations: List[str]) -> int:
        #i'll use a stack since operations will requires the last added values
        stack=[]

        for op in operations:
            #if is plus, we add to the stack the sum of the las two values added
            if op == '+':
                stack.append(stack[-1] + stack[-2])
            #if D add score that is the double of the previous score.
            elif op == 'D':
                stack.append(stack[-1] * 2)
            elif op == 'C':
                stack.pop()
            #At this point we now it must be a number so we cast it
            else:
                stack.append(int(op))
        return sum(stack)
        #Time & Space: O(n)
