...
class Solution:
    def summaryRanges(self, nums: List[int]) -> List[str]:
        ...
        while i < len(nums):
            ...
            #and we check the next number to see if it is consecutive
            while i < len(nums) - 1 and nums[i] + 1 == nums[i+1]:
                i += 1
            #no we need to check the interval length, so that if is >1 we add ->
            if start != nums[i]:
                ans.append(str(start)+ "->"+str(nums[i]))
            else:
                ans.append(str(nums[i]))
                
            #after appending we continue to the next value
            i += 1
        return ans
